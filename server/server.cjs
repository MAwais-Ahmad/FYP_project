...
// ─── GENERATE GENERAL APTITUDE TEST (AI semantic reasoning) ───────────────────
// The AI half of the hybrid aptitude test. It generates SEMANTIC reasoning MCQs —
// the kinds gpt-4o solves reliably (verbal logic, deduction) — plus open-ended
// reflection prompts. The exact-answer puzzle types (number/letter series, coding,
// direction, age, etc.) are generated by local formulas on the client instead,
// because LLMs systematically miscount those and this test is auto-graded.
...
const APTITUDE_CATEGORY_SLUGS = new Set(APTITUDE_CATEGORIES.map(c => c[0]));

// Aptitude questions are auto-graded, so answer-key correctness matters a lot.
// gpt-4o-mini frequently mislabels or malforms reasoning puzzles (wrong series
// answer, impossible clock angles, etc.), so both generation and verification use
// gpt-4o, which is markedly stronger at creating AND solving these. Bypasses
// callHighPrecisionAI (which forces gpt-4o-mini); falls back to OpenRouter gpt-4o.
const APTITUDE_MODEL = 'gpt-4o';
async function callAptitudeAI(params) {
  if (openaiClient) {
    try {
      return await openaiClient.chat.completions.create({ ...params, model: APTITUDE_MODEL });
    } catch (err) {
      console.warn(`⚠️ Aptitude OpenAI (${APTITUDE_MODEL}) failed: ${err.message}. Trying OpenRouter…`);
    }
  }
  if (openrouterClient) {
    return await openrouterClient.chat.completions.create({ ...params, model: 'openai/gpt-4o' });
  }
  throw new Error('No AI available for aptitude generation');
}

// Second-pass answer-key check: the generation model sometimes mislabels the
// correct option (its own worked solution can even contradict its answer). Here a
// fresh call RE-SOLVES each MCQ independently at temperature 0 and we overwrite
// correctAnswer with the verified letter. On any failure we keep the originals.
async function verifyAptitudeAnswers(mcqs) {
  if (!mcqs.length) return mcqs;
  const list = mcqs
    .map((q, i) => `${i}. ${q.question}\n${q.options.join('   ')}`)
    .join('\n\n');
  const prompt = `Solve each multiple-choice reasoning question below INDEPENDENTLY and carefully. Work out the single correct option for each and return ONLY its letter (A, B, C or D).

Respond ONLY with valid JSON: { "answers": [ { "i": 0, "letter": "A" } ] } — exactly one entry per question index.

QUESTIONS:
${list}`;
  try {
    const resp = await callAptitudeAI({
      messages: [{ role: 'user', content: prompt }],
      temperature: 0,
      max_tokens: 2000,
      response_format: { type: 'json_object' },
    ...
    for (const a of answers) {
      const i = Number(a.i);
      const letter = String(a.letter || '').trim().charAt(0).toUpperCase();
      if (Number.isInteger(i) && mcqs[i] && ['A', 'B', 'C', 'D'].includes(letter)) {
        mcqs[i].correctAnswer = letter;
      }
    }
  } catch (err) {
    console.warn('⚠️ Aptitude answer-verification failed, keeping original keys:', err.message);
  }
  return mcqs;
}
...
app.post('/api/generate-aptitude', async (req, res) => {
  try {
    let { mcqCount = 7, writtenCount = 3 } = req.body || {};
    mcqCount = Math.max(1, Math.min(15, Math.round(mcqCount)));
    writtenCount = Math.max(0, Math.min(5, Math.round(writtenCount)));

    const catList = APTITUDE_CATEGORIES.map(c => `"${c[0]}" (${c[1]})`).join(', ');

    const prompt = `You are an expert psychometric test designer. Create FRESH SEMANTIC / VERBAL logical-reasoning questions — the kind that make a person stop, focus and think. Every run must look different: randomise the wording, names, topics and WHICH categories appear.

GENERATE:
1) EXACTLY ${mcqCount} multiple-choice questions, each drawn from one of these reasoning categories (use a VARIETY — do not use any category more than twice): ${catList}.
2) EXACTLY ${writtenCount} open-ended written reflection / problem-solving questions (category "reflection") that reveal HOW the person thinks — their approach, judgement, values and self-awareness. These have NO single correct answer.

STRICT RULES:
- These MCQs must be SEMANTIC/VERBAL reasoning ONLY. Do NOT create number series, letter/alphabet series, coding-decoding, arithmetic, ages, dates/clocks, or distance/geometry questions (those are handled separately).
- NO general-knowledge, trivia, current-affairs or memorised-fact questions. Pure reasoning only.
- Each MCQ has EXACTLY 4 options formatted "A) …","B) …","C) …","D) …" and EXACTLY ONE correct option. SOLVE each one yourself and put the reasoning in "solution", then make sure "correctAnswer" is the letter of the genuinely correct option.
- Make 1–2 of the MCQs noticeably HARDER / trickier (an extra step of deduction, subtle wording, or very close distractors) to test adaptability — but keep them fair and unambiguous.
- Keep every question self-contained and clearly worded, suitable for university students (18–25). Names/scenarios may be neutral or South-Asian.
- Reflection questions: give one concrete scenario each and end by asking for the person's reasoning.

Respond ONLY with valid JSON in exactly this shape:
{
  "questions": [
    { "type": "mcq", "category": "number-series", "difficulty": "easy|medium|hard", "question": "…", "options": ["A) …","B) …","C) …","D) …"], "correctAnswer": "A", "solution": "…" },
    { "type": "text", "category": "reflection", "question": "…" }
  ]
}`;

    const response = await callAptitudeAI({
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.9,
      max_tokens: Math.min(16000, 1200 + (mcqCount + writtenCount) * 320),
      response_format: { type: 'json_object' },
    });
    ...
    for (const q of raw) {
      ...
      if (q.type === 'text' || q.category === 'reflection') {
        written.push({ type: 'text', category: 'reflection', question });
        continue;
      }
      const options = Array.isArray(q.options) ? q.options.map(String) : [];
      const letter = String(q.correctAnswer || '').trim().charAt(0).toUpperCase();
      if (options.length !== 4 || !['A', 'B', 'C', 'D'].includes(letter)) continue;
      mcqs.push({
        type: 'mcq',
        category: APTITUDE_CATEGORY_SLUGS.has(q.category) ? q.category : 'logical-consistency',
    ...
    if (mcqs.length === 0) {
      return res.status(502).json({ success: false, error: 'AI did not return usable aptitude questions.' });
    }

    // Correct any mislabelled answer keys before the (auto-graded) test ships.
    await verifyAptitudeAnswers(mcqs);

    const questions = [...mcqs.slice(0, mcqCount), ...written.slice(0, writtenCount)];
    totalTokensUsed += response.usage?.total_tokens || 0;
    console.log(`✅ Generated aptitude test (${mcqs.length} MCQ + ${written.length} written)`);
    res.json({ success: true, questions });
  } catch (err) {
    console.error('Generate aptitude error:', err.message);
    res.status(500).json({ success: false, error: 'Failed to generate aptitude test.' });
  }
});
